...
const ICON = { route: "🧭", worker: "🔧", tool: "🛠", evaluator: "🔎", llm: "✨", writer: "✍️" };
const fmt = (n, d = 2) => (n == null ? "—" : Number(n).toFixed(d));

/** Reference column next to the conversation: what was done for the last answer (steps with times), inference time, tokens, number of tool calls — and the session totals. */
    ...
    <aside className="ops">
      ...
      {last && (
        <>
          ...
          <ol className="steps">
            {last.steps.map((s, i) => (
              <li key={i} className={s.kind}>
                <span className="ic">{ICON[s.kind] || "•"}</span>
          ...
          <div className="src">answer: <b>{last.answer_mode || last.source || "—"}</b>{last.source && last.source !== last.answer_mode ? ` · source ${last.source}` : ""}{last.guard ? ` · guard ${String(last.guard).slice(0, 40)}` : ""}</div>
        </>
      )}
      <h4>This session</h4>
