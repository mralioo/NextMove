...
async def main() -> None:
    ...
    s = await ss.create_session(app_name="bench", user_id="u")
    runner = Runner(agent=agent, app_name="bench", session_service=ss)
    total = 0.0
    for label, q in QUESTIONS:
        ...
        dt = time.time() - t
        total += dt
        tm = (await ss.get_session(app_name="bench", user_id="u", session_id=s.id)).state.get("timing", {})
