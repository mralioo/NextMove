...
TASKS: dict[str, tuple[str, list[str], dict]] = {
    ...
    "ui-build": ("Build the React operator UI (frontend/ -> frontend/dist, served by the operator API at http://127.0.0.1:8770/app/)", ["bash", "-c", "cd frontend && npm install --no-audit --no-fund && npm run build"], {}),
    "ui-dev": ("React operator UI with hot reload on http://localhost:3000/app/ (proxies /api to the operator API on 8770; start it first with `make up`)", ["bash", "-c", "cd frontend && npm install --no-audit --no-fund && npm run dev"], {}),
