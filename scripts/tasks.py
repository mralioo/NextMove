...
TASKS: dict[str, tuple[str, list[str], dict]] = {
    ...
    "ui-build": ("Build the React operator desktop (frontend/ -> frontend/dist, served by the operator API at /app/)", ["bash", "-c", "cd frontend && npm install --no-audit --no-fund && npm run build"], {}),
    "ui-dev": ("React operator desktop with hot reload on http://localhost:5173 (proxies /api to the operator API)", ["bash", "-c", "cd frontend && npm install --no-audit --no-fund && npm run dev"], {}),
