...
End-to-end pipeline: raw training data -> the three normalized tables.
    ...
    python -m nextmove_pipeline.pipeline --data ./new_data --model data/normalized/normal_flow_model.pkl
...
def run(data_dir, out_dir, freq="15min", model_path=None, suffix="", offline=False,
    ...
    if model_path:
        ...
        episodes = build_episodes(load_events(data_dir), load_closures(data_dir),
                                  stations, list(flows.columns), adj, geocoder)
