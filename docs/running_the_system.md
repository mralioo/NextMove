...
Seven `make` commands. Everything else is a task listed by one script.

```bash
make install     # once: .venv + every dependency (dashboard, ML, MCP, agent, evaluation) + the UI build if npm exists
make up          # run everything (prepares the knowledge base, quality database, graph and UI first)
make down        # stop everything `make up` started
make check       # unit tests + guardrail suite + router accuracy — no LLM, no network
make ui          # operator UI with hot reload on http://localhost:3000/app/ (needs `make up` for the API)
make ui-build    # build the operator UI into frontend/dist (make up serves it at :8770/app/ and rebuilds it when the sources changed)
...
| **operator-api** | http://127.0.0.1:8770/app/ (**operator UI**; API docs `/docs`) | The API behind the UI: chat (also streaming), feedback loop, operator knowledge base, replay and analytics data — [`api_reference.md`](api_reference.md), [`operator_feedback_api.md`](operator_feedback_api.md) |
