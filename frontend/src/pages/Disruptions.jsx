...
export default function Disruptions() {
  ...
  const [selectedStations, setSelectedStations] = useState(['U Hallesches Tor']);
  const [timestamp, setTimestamp] = useState('2026-07-13 14:00:00');
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <p style={{ margin: '2px 0 0 0', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
              Simulates passenger spillover into adjacent lines based on shortest-path graph propagation (85% rerouting rate).
            </p>
          </div>
          <button
            onClick={runCascade}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div>
              <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'block', marginBottom: '6px' }}>
                Incident Timestamp (Local Berlin Time):
              </label>
              <input
                type="text"
                value={timestamp}
            ...
            {!cascadeResult ? (
              ...
              <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', fontSize: '0.78rem' }}>
                  <span>Spillover Overflow Ratio (1.0 = Baseline normal)</span>
                ...
                <div style={{ flex: 1, minHeight: 0 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={cascadeResult.stationsList} layout="vertical" margin={{ left: 10, right: 30, top: 5, bottom: 5 }}>
                      ...
                      <Tooltip formatter={(value) => [`${value}x Baseline`, 'Overflow Ratio']} />
